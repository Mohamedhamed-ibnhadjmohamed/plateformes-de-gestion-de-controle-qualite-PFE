...
@Component({
  selector: 'app-fin-chaine',
  templateUrl: './fin-chaine.component.html',
  styleUrl: '../shared/style/style.css',
})
export class FinChaineComponent implements OnInit {
  ...
  inspectionForm = new FormGroup({
    ...
    type_categorie: new FormControl('fin chaine', [Validators.required]),
  ...
  getProduit(): void {
    ...
    this.produitService.getAllById(this.idForm.value.idproduit).subscribe(
      (response: AllProduit[]) => {
        ...
        this.donneePourEnfant = this.allproduit;
      },
      (error: any) => {
